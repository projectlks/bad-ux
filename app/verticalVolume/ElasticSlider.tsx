...
const ElasticSlider: React.FC<ElasticSliderProps> = ({
  ...
  return (
    // UI ကို ရေပြင်ညီ (Horizontal) ပြန်ထားလိုက်ပါပြီ
    <div
...
const Slider: React.FC<SliderProps> = ({
  ...
  // Animation အတွက် ရေပြင်ညီ (left, right) ကို ပြန်သုံးပါမယ်
  ...
  // ဘယ်/ညာ ဆွဲဆန့်တဲ့ Elastic Animation အတွက်
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    if (e.buttons > 0 && sliderRef.current) {
      // ၁။ ရေပြင်ညီ Elastic Physics ဖြစ်ဖို့အတွက် Mouse ရဲ့ X ကို ပေးပို့ပါမယ်
      clientX.jump(e.clientX);

      // ၂။ ဒါပေမယ့် Value (အသံအတိုးအကျယ်) တွက်ချက်တာကိုတော့ အပေါ်/အောက် (Y ဝင်ရိုး) နဲ့ တွက်ပါမယ်
      const { top, height } = sliderRef.current.getBoundingClientRect();
      const percentage = 1 - (e.clientY - top) / height;
  ...
  return (
    <>
      <motion.div
        ...
        // ရေပြင်ညီ (Horizontal) အတိုင်းထားရှိခြင်း
        ...
        <div
          ...
          <motion.div
            ...
            className="flex grow">
            {/* အစ်ကို တောင်းဆိုထားတဲ့ အောက်ကနေ အပေါ်ကို ပြည့်သွားမယ့် UI Design */}
            <div className="relative w-full h-full overflow-hidden rounded-xl bg-gray-300">
              <div
                className="absolute bottom-0 w-full bg-gray-700 transition-all duration-75"
                style={{ height: `${getRangePercentage()}%` }}
              />
            </div>
          </motion.div>
        </div>
